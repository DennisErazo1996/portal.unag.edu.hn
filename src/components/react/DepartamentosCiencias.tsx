  ...
  return (
    <div className="mt-12">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Departamentos Académicos</h2>
        ...
        {departamentos.map((dept) => (
          <button
            ...
            className={[
              ...
              selectedId === dept.id
                ? "border-[#1ba333] bg-[#e8f5eb] dark:bg-unag-green/20 shadow-md"
                : "border-gray-200 dark:border-unag-green/30 bg-white dark:bg-unag-dark-green",
            ].join(" ")}
          >
            {dept.logo ? (
              <img
            ...
            <span
              className={[
                ...
                selectedId === dept.id
                  ? "text-[#145c24] dark:text-unag-green"
      ...
      {selected && (
        <div
          key={selected.id}
          className="mt-5 border-2 border-[#1ba333] rounded-2xl bg-white dark:bg-unag-dark-green p-8 flex flex-col sm:flex-row gap-8 items-center shadow-lg"
        ...
        >
          ...
          <div>
            <span className="inline-block bg-[#e8f5eb] dark:bg-unag-green/20 text-[#145c24] dark:text-unag-green border border-[#1ba333] text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full mb-3">
              Departamento Académico
            </span>
            <h3 className="text-xl font-bold text-[#145c24] dark:text-unag-green mb-3">{selected.nombre}</h3>
            <p className="text-sm text-gray-600 dark:text-white/80 leading-relaxed max-w-xl">
              {selected.descripcion}
            </p>
            <div className="flex flex-wrap gap-3 mt-5">
              <span className="flex items-center gap-1.5 text-xs bg-gray-100 dark:bg-background/50 border border-gray-200 dark:border-unag-green/20 text-gray-400 dark:text-white/50 px-3 py-1 rounded-full">
